...
Every InfluxDB use case is special and your [schema](/influxdb/v1.2/concepts/glossary/#schema) will reflect that uniqueness.
...
[Tags](/influxdb/v1.2/concepts/glossary/#tag) are indexed and [fields](/influxdb/v1.2/concepts/glossary/#field) are not indexed.
...
* Store data in fields if you plan to use them with an [InfluxQL function](/influxdb/v1.2/query_language/functions/)
* Store data in fields if you *need* them to be something other than a string - [tag values](/influxdb/v1.2/concepts/glossary/#tag-value) are always interpreted as strings
...
[Tags](/influxdb/v1.2/concepts/glossary/#tag) containing highly variable information like UUIDs, hashes, and random strings will lead to a large number of series in the database, known colloquially as high series cardinality.
High series cardinality is a primary driver of high memory usage for many database workloads.

See [Hardware Sizing Guidelines](/influxdb/v1.2/guides/hardware_sizing/#general-hardware-guidelines-for-a-single-node) for [series cardinality](/influxdb/v1.2/concepts/glossary/#series-cardinality) recommendations based on your hardware. If the system has memory constraints, consider storing high-cardinality data as a field rather than a tag.

### *Don't encode data in measurement names*

In general, taking this step will simplify your queries.
InfluxDB queries merge data that fall within the same [measurement](/influxdb/v1.2/concepts/glossary/#measurement); it's better to differentiate data with [tags](/influxdb/v1.2/concepts/glossary/#tag) than with detailed measurement names.
...
Shard groups are organized by [retention policy](/influxdb/v1.2/concepts/glossary/#retention-policy-rp) (RP) and store data with timestamps that fall within a specific time interval.
The length of that time interval is called the [shard group duration](/influxdb/v1.2/concepts/glossary/#shard-duration).

By default, the shard group duration is determined by the RP's [duration](/influxdb/v1.2/concepts/glossary/#duration):
...
See [Retention Policy Management](/influxdb/v1.2/query_language/database_management/#retention-policy-management) for how to configure the
...
> **Note:** Note that `INF` (infinite) is not a valid duration [when configuring](/influxdb/v1.2/query_language/database_management/#retention-policy-management)
...
* each shard group has at least 100,000 [points](/influxdb/v1.2/concepts/glossary/#point) per shard group
* each shard group has at least 1,000 points per [series](/influxdb/v1.2/concepts/glossary/#series)
